...
export default function LoginPage() {
  ...
  // NOUVEAU : Vraie connexion Google
    ...
    <div className={`min-h-screen font-sans text-left italic font-black uppercase tracking-tighter transition-colors duration-300 flex flex-col justify-center items-center p-4 md:p-8 ${isDark ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <div className={`w-full max-w-lg p-8 md:p-12 rounded-[3rem] border-[6px] shadow-[16px_16px_0px_0px_rgba(0,0,0,1)] ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-900'}`}>
        ...
        <div className="flex flex-col gap-4 mb-8">
            ...
            <button onClick={handleGoogleLogin} disabled={loading} className={`w-full py-4 rounded-2xl border-4 flex items-center justify-center gap-3 transition-all shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-y-1 hover:shadow-none disabled:opacity-50 ${isDark ? 'bg-slate-800 border-slate-600 text-white hover:bg-slate-700' : 'bg-white border-slate-900 text-slate-900 hover:bg-slate-50'}`}>
            ...
            {/* BOUTON APPLE MIS À JOUR */}
            <button onClick={handleAppleLogin} disabled={loading} className={`w-full py-4 rounded-2xl border-4 flex items-center justify-center gap-3 transition-all shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-y-1 hover:shadow-none disabled:opacity-50 ${isDark ? 'bg-slate-950 border-slate-700 text-white hover:bg-black' : 'bg-slate-900 border-slate-900 text-white hover:bg-slate-800'}`}>
